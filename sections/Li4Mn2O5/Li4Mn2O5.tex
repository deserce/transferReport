...
\section{Background} %TODO Rewrite/extend Chapter 3 background
Defect chemistry, and the pathways and energetics that govern ion diffusion in electrodes are of vital importance to understanding the electrochemical performance of a potential battery material.
These parameters can not be determined by experimental methods alone, requiring atomistic or DFT studies to study migration barriers and understand the direction and dimensionality of lithium motion.
...
This section consists of the first atomistic defect and dynamics studies of \ce{Li4Mn2O5}, addressing issues arising due to the highly disordered nature of the material.
\newpage
\begin{figure}
\centering
...
\end{figure}
\section{Structure generation and modelling} %TODO Extend
The structure of \ce{Li4Mn2O5}, shown in Figure \ref{fig:Li4Mn2O5-average}, has an average \ce{MnO} rock-salt structure with the random substitution of 2/\nth{3}\textsuperscript{s} of \ce{Mn} sites with Li, charge compensated by 1/\nth{6} of oxygen sites being vacant.
...
It is noted in the literature that local deviations from the rock-salt structure arising from structural disorder give rise to a broad range of interatomic spacings.\cite{Yahia2019}
...
\subsection{Disordered structure generation} %TODO Review disordered structure generation
In the absence of prior information regarding the energetics or stability of given local environments, nor a means of procedurally generating and analysing structures, random generation of structures is performed.
Twenty random structures were generated, half containing 216 atoms and the other half containing 1728 atoms (to conserve stoichiometry), with each site randomly assigned an atom in accordance with the occupancy of that site.
...
Table \ref{tab:randomresults} gives the lattice parameters of each of the relaxed random structures which converged, with cell lengths scaled for comparison with experimental data.\cite{Freire2016}
Whilst some structures do give good agreement with experimental lattice parameters (e.g. 1728 atoms, trial 9), some other randomly generated structures have cell lengths which differ from experiment by over 7 percent, suggesting that random structure generation does not produce structures which reliably agree with experiment.
...
\cmidrule(lr){2-4}
\textbf{Site (n)} & \mc{$\ce{V_{Li_n}}$} & \mc{$\ce{V_{Mn_n}}$} & \mc{$\ce{V_{O_n}}$}\\
\midrule
1 & 6.93 & 46.19 & 24.77 \\
2 & 5.57 & 46.19 & 23.24 \\
...
&\multicolumn{3}{c}{Energy (eV)}\\
\cmidrule(lr){2-4}
\textbf{Site (n)} & \mc{$\ce{Li_{i_n}}$} & \mc{$\ce{Mn_{i_n}}$} & \mc{$\ce{O_{i_n}}$}\\
\midrule
...
1            &            & 0.32       & 1.68       & \tableline & \tableline & \tableline \\   
2            & 1.68       &            & 2.13       & \tableline & \tableline & \tableline \\
3            & 1.29       & 0.45       &            & 0.61       & \tableline & \tableline \\
4            & \tableline & \tableline & 0.61       &            & 1.29       & 0.45       \\
5            & \tableline & \tableline & \tableline & 1.61       &            & 0.32       \\
6            & \tableline & \tableline & \tableline & 2.13       & 1.68       &            \\
\bottomrule
\end{tabular}
}
\label{tab:migration}
...
As \ce{Li4Mn2O5} is a disordered rock-salt, isotropic diffusion throughout the bulk material is expected, even if there is some degree of anisotropy locally.
...
A system consisting of 11264 ions (an 8x8x8 supercell of the ground structure proposed by \citet{Diaz-Lopez2017}) had 10\% of lithium ions removed (10854 ions) at random, charge compensated by the reduction of \ce{Mn^3+ -> Mn^4+}.
...
Figure \ref{fig:msd1000} shows the MSD of the system at 1000K, split into ion motion in Li1/Li2 and Li3/Li4 grids (ac plane) and diffusion onto and between sites 3 and 4 (b-direction).
...
\newpage
\section{Chapter summary and conclusions} %TODO Review chapter summary
This chapter provides atomic scale insights into the energetics of defect formation and lithium ion migration in \ce{Li4Mn2O5}.
...
\begin{itemize}
	\item Structural modelling can be performed using either the mean-field approximation or ordered approximations of \ce{Li4Mn2O5}, but purely random structure generation produces unphysical systems
